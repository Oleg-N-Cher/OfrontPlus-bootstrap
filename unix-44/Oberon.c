...
export void **Oberon_ParRec__typ;
...
static void Oberon_PopulateParams (void)
{
  ...
  do {
    if ((INTEGER)i <= CmdArgs_Count) {
      CmdArgs_Get(i, (void*)str, 32);
      Texts_WriteString(&W, Texts_Writer__typ, (void*)str, 32);
      Texts_WriteString(&W, Texts_Writer__typ, (CHAR*)" ", 2);
    }
    i += 1;
  } while (!((INTEGER)i > CmdArgs_Count));
...
static void EnumPtrs(void (*P)(void*))
{
  P(Oberon_Log);
  P(Oberon_Par);
  __ENUMR(&Oberon_R, Texts_Reader__typ, 52, 1, P);
  __ENUMR(&Oberon_W, Texts_Writer__typ, 40, 1, P);
}

__TDESC(Oberon_ParRec__desc, 2, 1) = {__TDFLDS("ParRec", 8), {0, -8}};
