...
typedef
  CHAR (*CmdArgs_ArgPtr)[2048];

typedef
  CmdArgs_ArgPtr (*CmdArgs_ArgVec)[2048];

typedef
  CHAR (*CmdArgs_EnvPtr)[2048];
...
void CmdArgs_Get (INTEGER n, CHAR *val, INTEGER val__len)
{
  ...
  if (n < CmdArgs_argc()) {
    av = CmdArgs_argv();
    if (val__len > __STRLEN(*(*av)[n], 2048, (CHAR*)"CmdArgs", 7423)) {
      __STRCOPYCC(*(*av)[n], val, __MIN(val__len, 2048), (CHAR*)"CmdArgs", 7220);
    }
  }
}
...
void CmdArgs_GetEnv (CHAR *var, INTEGER var__len, CHAR *val, INTEGER val__len)
{
  ...
  if (p != NIL) {
    __STRCOPYCC(*p, val, __MIN(val__len, 2048), (CHAR*)"CmdArgs", 15134);
  } else {
    val[0] = 0x00;
  }
}
...
void CmdArgs_GetEnvN (INTEGER n, CHAR *s, INTEGER s__len)
{
  ...
  if (p->e != NIL) {
    __STRCOPYCC(*p->e, s, __MIN(s__len, 2048), (CHAR*)"CmdArgs", 20768);
  } else {
    s[0] = 0x00;
  }
}
