...
static void OfrontOPV_StringCopy (OfrontOPT_Node left, OfrontOPT_Node right, BOOLEAN exp)
{
  __ASSERT((INTEGER)right->class != 7, -1, (CHAR*)"OfrontOPV", 305182);
  if ((INTEGER)right->class == 12) {
    __ASSERT((INTEGER)right->subcl == 8, -1, (CHAR*)"OfrontOPV", 305693);
    ...
    while ((INTEGER)right->class == 12) {
      __ASSERT((INTEGER)right->subcl == 8, -1, (CHAR*)"OfrontOPV", 307486);
...
static void OfrontOPV_compStat (OfrontOPT_Node n, BOOLEAN exp)
{
  ...
  while (n != NIL && OfrontOPM_noerr) {
    ...
    __ASSERT((INTEGER)n->class == 19, -1, (CHAR*)"OfrontOPV", 311580);
    l = n->left;
    r = n->right;
    if ((INTEGER)n->subcl == 0) {
      if (__IN(r->typ->form, 0x040400)) {
        ...
        __ASSERT(__IN(l->typ->form, 0x022008), -1, (CHAR*)"OfrontOPV", 314425);
        OfrontOPV_design(l, -1);
        OfrontOPM_WriteString((CHAR*)" = ", 4);
        OfrontOPV_expr(r, -1);
      }
    } else {
      __ASSERT((INTEGER)n->subcl == 1, -1, (CHAR*)"OfrontOPV", 315167);
      __ASSERT((INTEGER)l->typ->BaseTyp->comp == 3, -1, (CHAR*)"OfrontOPV", 315431);
...
static void OfrontOPV_stat (OfrontOPT_Node n, OfrontOPT_Object outerProc)
{
  ...
  while (n != NIL && OfrontOPM_noerr) {
    ...
    switch (n->class) {
      case 18: 
        __ASSERT(n->obj != NIL, -1, (CHAR*)"OfrontOPV", 325914);
        proc = n->obj;
