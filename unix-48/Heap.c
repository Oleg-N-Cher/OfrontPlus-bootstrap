...
extern void Platform_OSFree (SYSTEM_ADRINT address);
#define Heap_HeapModuleInit()  Heap__init()
#define Heap_OSAllocate(size)  Platform_OSAllocate(size)
#define Heap_OSFree(address)  Platform_OSFree(address)
...
SYSTEM_PTR Heap_REGMOD (CHAR *name, INTEGER name__len, Heap_EnumProc enumPtrs)
{
  ...
  m->types = NIL;
  m->cmds = NIL;
  __STRCOPYCC(name, m->name, __MIN(32, name__len), (CHAR*)"Heap", 37139);
  m->refcnt = 0;
  m->enumPtrs = enumPtrs;
...
__TDESC(Heap_ModuleDesc__desc, 2, 3) = {__TDFLDS("ModuleDesc", 60), {0, 40, 44, -16}};
__TDESC(Heap_CmdDesc__desc, 2, 1) = {__TDFLDS("CmdDesc", 32), {0, -8}};
