...
REAL bbMath_Sqrt (REAL x)
{
  ...
  if (x == (REAL)0) {
    ...
    __ASSERT(x > (REAL)0, -1, (CHAR*)"bbMath", 24593);
...
REAL bbMath_Exp (REAL x)
{
  REAL xn, g, p, q, z;
  INTEGER n;
  __ASSERT(x <= bbMath_LnInfinity, -1, (CHAR*)"bbMath", 31769);
  if (x < bbMath_LnSmall) {
    return (REAL)0;
...
REAL bbMath_Ln (REAL x)
{
  ...
  INTEGER n;
  __ASSERT(x > (REAL)0, -1, (CHAR*)"bbMath", 38672);
...
REAL bbMath_Tan (REAL x)
{
  ...
  __ASSERT(__ABS(cos) >= 5.5626846462680078e-309, -1, (CHAR*)"bbMath", 63002);
  return sin / cos;
}
