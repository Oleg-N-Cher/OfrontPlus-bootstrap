    ...
    if (count >= maxCount || curLen != nextLen) {
      if (count < minCount) {
        do {
          ...
          ZlibDeflate_SendCode(stream, stream__typ, &(_ptr__62->data)[__X(curLen, _ptr__62->len[0], (CHAR*)"ZlibDeflate", 0)], ZlibDeflate_Node__typ);
          count -= 1;
        } while (!(count == 0));
      } else if (curLen != 0) {
        if (curLen != prevLen) {
          _ptr__61 = (*stream).bnode;
          ZlibDeflate_SendCode(stream, stream__typ, &(_ptr__61->data)[__X(curLen, _ptr__61->len[0], (CHAR*)"ZlibDeflate", 0)], ZlibDeflate_Node__typ);
          count -= 1;
        }
        ...
        ZlibDeflate_SendCode(stream, stream__typ, &(_ptr__59->data)[__X(17, _ptr__59->len[0], (CHAR*)"ZlibDeflate", 0)], ZlibDeflate_Node__typ);
        ZlibDeflate_SendBits(stream, stream__typ, count - 3, 3);
      } else {
        _ptr__58 = (*stream).bnode;
        ZlibDeflate_SendCode(stream, stream__typ, &(_ptr__58->data)[__X(18, _ptr__58->len[0], (CHAR*)"ZlibDeflate", 0)], ZlibDeflate_Node__typ);
...
static void EnumPtrs(void (*P)(void*))
{
  ...
  __ENUMR(&ZlibDeflate_LTree, ZlibDeflate_StaticTree__typ, 24, 1, P);
  __ENUMR(&ZlibDeflate_DTree, ZlibDeflate_StaticTree__typ, 24, 1, P);
  __ENUMR(&ZlibDeflate_BTree, ZlibDeflate_StaticTree__typ, 24, 1, P);
}

__TDESC(ZlibDeflate_Node__desc, 2, 0) = {__TDFLDS("Node", 4), {-8}};
__TDESC(ZlibDeflate_StaticTree__desc, 1, 2) = {__TDFLDS("StaticTree", 24), {0, 8, -24}};
__TDESC(ZlibDeflate_Tree__desc, 2, 3) = {__TDFLDS("Tree", 40), {0, 16, 24, -32}};
__TDESC(ZlibDeflate_PendingBuffer__desc, 2, 1) = {__TDFLDS("PendingBuffer", 16), {0, -16}};
__TDESC(ZlibDeflate_Stream__desc, 1, 18) = {__TDFLDS("Stream", 2672), {80, 96, 104, 144, 160, 176, 184, 200, 216, 224, 240, 256, 264, 280, 288, 296, 
  2632, 2640, -152}};
__TDESC(ZlibDeflate__1__desc, 2, 0) = {__TDFLDS("", 16), {-8}};
