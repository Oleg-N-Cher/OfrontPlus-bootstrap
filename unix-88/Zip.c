...
static Zip_Entry Zip_ReadEntry (Files_Rider *r, void **r__typ)
{
  ...
  if (longDummy == 33639248) {
    ...
    if (extraLen != 0) {
      ...
      Files_ReadBytes(r, r__typ, (void*)(SYSTEM_ADRINT)ent->extraField->data, (LONGINT)ent->extraField->len[0], extraLen);
    }
    if (commentLen > 0) {
      ent->comment = __NEWARR(NIL, 1, 1, 1, 1, (SYSTEM_ARRLEN)commentLen);
      Files_ReadBytes(r, r__typ, (void*)(SYSTEM_ADRINT)ent->comment->data, (LONGINT)ent->comment->len[0], commentLen);
    }
...
static void Zip_WriteLocalFileHeader (Zip_Entry ent, Files_Rider *r, void **r__typ)
{
  ...
  if (ent->extraField != NIL) {
    Files_WriteBytes(r, r__typ, (void*)(SYSTEM_ADRINT)ent->extraField->data, (LONGINT)ent->extraField->len[0], ent->extraField->len[0]);
  }
}
...
static void Zip_WriteFileHeader (Zip_Entry ent, Files_Rider *r, void **r__typ)
{
  ...
  if (ent->comment != NIL) {
    Files_WriteBytes(r, r__typ, (void*)(SYSTEM_ADRINT)ent->comment->data, (LONGINT)ent->comment->len[0], ent->comment->len[0]);
  }
}
...
static void Zip_WriteEndOfCentDir (Zip_Archive arc, Files_Rider *r, void **r__typ)
{
  ...
  if (arc->comment == NIL) {
    ...
    Files_WriteBytes(r, r__typ, (void*)(SYSTEM_ADRINT)arc->comment->data, (LONGINT)arc->comment->len[0], arc->comment->len[0]);
  }
}
    ...
    if (longDummy != 101010256) {
      *res = -2;
      ...
      if (intDummy != 0) {
        ...
        Files_ReadBytes(&r, Files_Rider__typ, (void*)(SYSTEM_ADRINT)arc->comment->data, (LONGINT)arc->comment->len[0], intDummy);
      }
...
void Zip_DeleteEntry (Zip_Archive arc, Zip_Entry *ent, INTEGER *res)
{
  ...
  if (*res == 0) {
    ...
    Files_Close(f);
    __ASSERT(ent2 == (*ent)->next, -1, (CHAR*)"Zip", 160025);
    if (ent2 != NIL) {
...
void Zip_ReadBytes (Zip_Reader r, CHAR *buf, INTEGER buf__len, INTEGER offset, INTEGER len, INTEGER *read)
{
  ...
  if (r->open) {
    if (__ISP(r, Zip_UncompReaderDesc, 1)) {
      if (offset == 0) {
        Files_ReadBytes(&(__GUARDP(r, Zip_UncompReaderDesc, 1))->fr, Files_Rider__typ, (void*)(SYSTEM_ADRINT)buf, (LONGINT)buf__len, len);
      } else {
        bufp = __NEWARR(NIL, 1, 1, 1, 1, (SYSTEM_ARRLEN)len);
        Files_ReadBytes(&(__GUARDP(r, Zip_UncompReaderDesc, 1))->fr, Files_Rider__typ, (void*)(SYSTEM_ADRINT)bufp->data, (LONGINT)bufp->len[0], len);
        _for__18 = len - 1;
        i = 0;
        while (i <= _for__18) {
...
/*----------------------------------------------------------------------------*/
__TDESC(Zip_EntryDesc__desc, 2, 3) = {__TDFLDS("EntryDesc", 328), {288, 296, 320, -32}};
__TDESC(Zip_ArchiveDesc__desc, 2, 4) = {__TDFLDS("ArchiveDesc", 48), {8, 16, 32, 40, -40}};
__TDESC(Zip_ReaderDesc__desc, 2, 1) = {__TDFLDS("ReaderDesc", 16), {8, -16}};
__TDESC(Zip_UncompReaderDesc__desc, 2, 2) = {__TDFLDS("UncompReaderDesc", 56), {8, 24, -24}};
__TDESC(Zip_DefReaderDesc__desc, 2, 10) = {__TDFLDS("DefReaderDesc", 640), {8, 40, 64, 144, 160, 552, 560, 584, 600, 616, -88}};
