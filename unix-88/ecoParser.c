...
{
  __ASSERT((*prs).ctxIdx < (*prs).ctxStk->len[0], -1, (CHAR*)"ecoParser", 15164);
...
{
  __ASSERT((*prs).ctxIdx != 0, -1, (CHAR*)"ecoParser", 17198);
  (*prs).ctxIdx -= 1;
...
void ecoParser_Parser_Init (ecoParser_Parser *prs, void **prs__typ, ecoParser_StrPtr buf, INTEGER bufLen, INTEGER ctxDeep)
{
  ...
  if (ctxDeep != 0) {
    (*prs).ctxStk = __NEWARR(POINTER__typ, 8, 8, 1, 1, (SYSTEM_ARRLEN)ctxDeep);
    (*prs).ctxIdx = 0;
  }
}
...
/*----------------------------------------------------------------------------*/
__TDESC(ecoParser_Tree__desc, 8, 3) = {__TDFLDS("Tree", 24), {0, 8, 16, -32}};
__TDESC(ecoParser_Parser__desc, 7, 4) = {__TDFLDS("Parser", 48), {0, 16, 24, 40, -40}};
__TDESC(ecoParser_Node__desc, 2, 2) = {__TDFLDS("Node", 40), {0, 24, -24}};
__TDESC(ecoParser_CtxNode__desc, 3, 3) = {__TDFLDS("CtxNode", 56), {0, 24, 40, -32}};
__TDESC(ecoParser_MethodNode__desc, 2, 2) = {__TDFLDS("MethodNode", 40), {0, 24, -24}};
__TDESC(ecoParser_SetCtxNode__desc, 2, 3) = {__TDFLDS("SetCtxNode", 48), {0, 24, 40, -32}};
