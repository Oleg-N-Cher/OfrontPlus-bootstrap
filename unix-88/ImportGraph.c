...
static void ImportGraph_OutputImports (BOOLEAN reduced)
{
  ...
  while (i <= CmdArgs_Count) {
    ...
    if (__STRCMPCC(ImportGraph_arg, (CHAR*)"-r", 3, (CHAR*)"ImportGraph", 124166) == 0) {
      reduced = !reduced;
    } else {
...
{
  P(ImportGraph_f);
  __ENUMR(&ImportGraph_r, Files_Rider__typ, 32, 1, P);
  P(ImportGraph_first);
}

__TDESC(ImportGraph_ModuleDesc__desc, 2, 2) = {__TDFLDS("ModuleDesc", 88), {64, 80, -24}};
__TDESC(ImportGraph_ModuleRefDesc__desc, 2, 2) = {__TDFLDS("ModuleRefDesc", 16), {0, 8, -24}};
__TDESC(ImportGraph__1__desc, 1, 0) = {__TDFLDS("", 12), {-8}};
