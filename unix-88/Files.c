...
static void Files_Deregister (CHAR *name, INTEGER name__len)
{
  ...
  if (Platform_IdentifyByName((void*)name, name__len, &identity) == 0) {
    ...
    if (osfile != NIL) {
      __ASSERT(!osfile->tempFile, -1, (CHAR*)"Files", 34592);
      __ASSERT(osfile->fd >= 0, -1, (CHAR*)"Files", 34616);
...
static void Files_Create (Files_File f)
{
  ...
  if (f->fd == Platform_InvalidHandleValue()) {
    if ((INTEGER)f->state == 1) {
      ...
      __ASSERT((INTEGER)f->state == 2, -1, (CHAR*)"Files", 41759);
...
void Files_ReadBytes (Files_Rider *r, void **r__typ, BYTE *x, INTEGER x__len, INTEGER n)
{
  ...
  while (n > 0) {
    ...
    __MOVE((SYSTEM_ADRINT)buf->data + (LONGINT)offset, (SYSTEM_ADRINT)x + (LONGINT)xpos, min);
    offset += min;
    (*r).offset = offset;
    xpos += min;
    n -= min;
...
void Files_WriteBytes (Files_Rider *r, void **r__typ, BYTE *x, INTEGER x__len, INTEGER n)
{
  ...
  while (n > 0) {
    ...
    __MOVE((SYSTEM_ADRINT)x + (LONGINT)xpos, (SYSTEM_ADRINT)buf->data + (LONGINT)offset, min);
    offset += min;
    (*r).offset = offset;
    if (offset > buf->size) {
...
void Files_WriteString (Files_Rider *R, void **R__typ, CHAR *x, INTEGER x__len)
{
  ...
  Files_WriteBytes(R, R__typ, (void*)(SYSTEM_ADRINT)x, (LONGINT)x__len, i + 1);
}
...
__TDESC(Files_FileDesc__desc, 2, 4) = {__TDFLDS("FileDesc", 616), {568, 576, 584, 592, -40}};
